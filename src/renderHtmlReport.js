 ...
 * 
 * Refactored for standalone Npm Package usage.
 */
...
const generateHtmlReport = (options = {}) => {
    console.log("🔨 Starting HTML Report Build...");

    const PATHS = {
        results: options.resultsPath || path.join(process.cwd(), 'cypress/reports/results.json'),
        baseResults: options.baseResultsPath || path.join(process.cwd(), 'cypress/reports/.jsons/results.json'),
        reportsFolder: options.reportsFolder || path.join(process.cwd(), 'cypress/reports/.jsons'),
        template: path.join(__dirname, '../ui/index.html'),
        styles: path.join(__dirname, '../ui/styles.css'),
        app: path.join(__dirname, '../ui/app.js'),
        outputDir: options.outputDir || path.join(process.cwd(), 'cypress/reports/html'),
        outputFile: options.outputFile || 'testaignite-report.html'
    };

    // 1. Data Ingestion (Merged Mochawesome JSON parts)
    const getResults = () => {
        console.log("🔍 Scanning for report parts in .jsons folder...");
        const reportsFolder = path.resolve(PATHS.reportsFolder).replace(/\\/g, '/');
        ...
        files.forEach(f => {
            ...
            // Use the latest meta
            master.meta = data.meta || master.meta;
        });
    ...
    if (!source) {
        ...
        console.error("Please ensure the 'ui' directory is included in your package distribution.");
        return;
    }
